...
func main() {
	...
	switch {
	case weight > desiredWeight:
		fmt.Println("Sports should be continued")
	case weight == desiredWeight:
		fmt.Println("the desired weight has been reached")
	case weight < desiredWeight:
		fmt.Println("lost more weight than desired")
	default:
		fmt.Println("no measurement done")
